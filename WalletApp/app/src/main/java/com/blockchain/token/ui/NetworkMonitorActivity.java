...
public final class NetworkMonitorActivity extends AbstractWalletActivity
{
	...
	protected void onCreate(final Bundle savedInstanceState)
	{
		...
		if (pager != null)
		{
			final RadioGroup group = (RadioGroup) findViewById(R.id.network_monitor_group);
			group.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
				@Override
				public void onCheckedChanged(RadioGroup group, int checkedId) {
					if(checkedId==R.id.peerList){
			...
			pager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
				...
				public void onPageSelected(int position) {
					if(position == 0){
						group.check(R.id.peerList);
					}else {
						group.check(R.id.blockList);
					}
				}
				@Override
