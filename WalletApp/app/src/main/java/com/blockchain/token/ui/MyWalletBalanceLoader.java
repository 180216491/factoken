...
import com.blockchain.token.util.ThrottlingWalletChangeListener;
 ...
 */
public final class MyWalletBalanceLoader extends AsyncTaskLoader<Coin[]>
{
	private LocalBroadcastManager broadcastManager;
	private final Wallet wallet;

	private static final Logger log = LoggerFactory.getLogger(MyWalletBalanceLoader.class);

	public MyWalletBalanceLoader(final Context context, @Nonnull final Wallet wallet)
	{
		super(context);
	...
	{
		Coin value = wallet.getBalance(BalanceType.ESTIMATED);
		Coin enable = wallet.getBalance(BalanceType.AVAILABLE);
		Coin[] arrays = new Coin[2];
		arrays[0] = value;
		arrays[1] = enable;
		return arrays;
	}
	...
	private void safeForceLoad()
	{
		...
		{
			log.info("rejected execution: " + MyWalletBalanceLoader.this.toString());
		}
	}
}
